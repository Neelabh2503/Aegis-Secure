...
class TestSMSSpamPredictionIntegration:
    ...
    def test_sms_imports_spam_prediction(self):
        """Test SMS module imports get_spam_prediction from notifications."""
        # Verify SMS module uses shared spam prediction
...
class TestSMSModels:
    ...
    def test_sms_sync_request_model(self):
        ...
        messages = [
            sms.SmsMessage(address="111", body="Msg1", date_ms=1000, type="inbox"),
            sms.SmsMessage(address="222", body="Msg2", date_ms=2000, type="sent")
        ]
        req = sms.SmsSyncRequest(messages=messages)
    ...
    def test_sms_message_validation(self):
        ...
        with pytest.raises(ValueError):
            sms.SmsMessage(address="123")  # Missing required fields
...
class TestSyncSMS:
    ...
    async def test_sync_sms_new_messages(self):
        ...
        mock_user = {"user_id": "user123"}
        messages = [
            sms.SmsMessage(address="111", body="Test1", date_ms=1000, type="inbox"),
            sms.SmsMessage(address="222", body="Test2", date_ms=2000, type="sent")
        ]
        request = sms.SmsSyncRequest(messages=messages)
    ...
    async def test_sync_sms_duplicate_detection(self):
        ...
        mock_user = {"user_id": "user123"}
        messages = [
            sms.SmsMessage(address="111", body="Test", date_ms=1000, type="inbox")
        ]
        request = sms.SmsSyncRequest(messages=messages)
    ...
    async def test_sync_sms_no_user_id(self):
        ...
        messages = [sms.SmsMessage(address="111", body="Test", date_ms=1000, type="inbox")]
        request = sms.SmsSyncRequest(messages=messages)
...
class TestRetryFailedSMSPredictions:
    """Test retry mechanism for failed SMS predictions."""
    
    @pytest.mark.asyncio
    async def test_retry_processes_unknown_verdicts(self):
        """Test retry processes messages with unknown verdicts."""
        mock_sms = {
            "_id": "sms123",
            "address": "111",
            "body": "Test message",
        ...
        with patch('routes.sms.sms_messages_col.find', return_value=MockCursor([mock_sms])), \
             patch('routes.sms.get_spam_prediction', new_callable=AsyncMock, return_value=mock_prediction), \
             patch('routes.sms.sms_messages_col.update_one', new_callable=AsyncMock) as mock_update, \
             patch('routes.sms.asyncio.sleep', new_callable=AsyncMock, side_effect=StopAsyncIteration):
            
            try:
                await sms.retry_failed_sms_predictions()
            except StopAsyncIteration:
                pass  # Expected - breaks the infinite loop
            
            mock_update.assert_called_once()
    
    @pytest.mark.asyncio
    async def test_retry_handles_prediction_errors(self):
        """Test retry handles prediction failures gracefully."""
        mock_sms = {
            "_id": "sms123",
            "address": "111",
            "body": "Test",
        ...
        with patch('routes.sms.sms_messages_col.find', return_value=MockCursor([mock_sms])), \
             patch('routes.sms.get_spam_prediction', new_callable=AsyncMock, side_effect=Exception("API Error")), \
             patch('routes.sms.sms_messages_col.update_one', new_callable=AsyncMock) as mock_update, \
             patch('routes.sms.asyncio.sleep', new_callable=AsyncMock, side_effect=StopAsyncIteration), \
