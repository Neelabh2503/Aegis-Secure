...
class EmailDetailScreen extends StatelessWidget {
  ...
  Map<String, String> parseSender(String sender) {
    ...
    if (match != null) {
      return {
        'name': match.group(1)?.trim() ?? sender,
        'email': match.group(2)?.trim() ?? sender,
      };
    } else {
      // If no angle brackets, fallback to same string for name & email
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            parseSender(email.sender)['email'] ?? "(No Email)",
